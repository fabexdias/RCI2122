				...
				/* Este if vai até à linha 963 e trata de todas os comandos inseridos pelo utilizador local, descritos da linha 129 à linha 139 */	
					...
					if(fd_tcp != -1 && fd_udp != -1){
						...
						if(!strncmp(buffer,"pentry",6) || !strcmp(buffer,"new\n") || !strcmp(buffer,"exit\n") || !strncmp(buffer,"p",1) || !strcmp(buffer,"n\n") || !strcmp(buffer,"e\n") || !strcmp(buffer,"b\n") || !strncmp(buffer,"bentry",6)){
							...
							if(shortcut.fd != -1){
								...
								if(sscanf(buffer,"%s %s %s %s", garbage, shortcut.key, shortcut.ip, shortcut.port) == 4){
									if(key_check(shortcut.key) != -1 && port_check(shortcut.port) != -1){								
										...
										// Verificar se o anel não é um anel de um só nó, verificar se o anel é composto por mais que 2 nós, ou caso existam apenas dois se o atalho é para o predecessor/sucessor
										if(one_node_ring == 0 && (successor.fd != predecessor.fd || (successor.fd == predecessor.fd && strcmp(shortcut.key, predecessor.key) == 0 && strcmp(shortcut.ip, predecessor.ip) == 0 && strcmp(shortcut.port, predecessor.port) == 0))){
											if((!(key_check(local.key) < key_check(shortcut.key) && key_check(shortcut.key) < key_check(successor.key)) && key_check(local.key) < key_check(successor.key)) || (!(key_check(local.key) > key_check(shortcut.key) && key_check(shortcut.key) < key_check(successor.key)) && !(key_check(local.key) < key_check(shortcut.key) && key_check(shortcut.key) > key_check(successor.key)) && key_check(local.key) > key_check(successor.key))){
												...
												node_setup(&shortcut, "\0", "\0", "\0", -1);
												printf("[*] Atalho não válido. Não está presente no anel\n");
											}
										}else{
											node_setup(&shortcut, "\0", "\0", "\0", -1);
											printf("[*] Atalho não válido. Não está presente no anel\n");
							...
							printf("[*] Use s/show, f/find, c/chord, d/dchord ou l/leave\n");
						}
